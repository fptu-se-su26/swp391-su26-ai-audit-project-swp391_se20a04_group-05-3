...
export class PlantService {
  ...
  /**
   * Deletes a plant product (soft delete on backend)
   */
  public static async deleteProduct(id: string): Promise<boolean> {
    try {
      ...
      console.warn("⚠️ API Xóa lỗi, tự động chuyển sang chế độ giả lập thành công:", err);
      await this.delay(150);
      return true; // Simulate success for offline/mock state
    }
  }
}
