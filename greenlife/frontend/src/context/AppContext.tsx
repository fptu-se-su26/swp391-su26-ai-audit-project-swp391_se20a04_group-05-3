...
import { UserService, DashboardUser } from "../services/userService";
...
interface AppContextType {
  ...
  register: (name: string, email: string, role: "customer" | "store" | "admin", password?: string) => Promise<{ success: boolean; message: string }>;
  ...
  resetPasswordVerify: (email: string, otp: string, newPassword: string) => Promise<{ success: boolean }>;
  logout: () => Promise<void>;
  ...
  users: DashboardUser[];
  awardUserCredits: (userId: string, amount: number) => Promise<boolean>;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const registerVerify = async (email: string, otp: string) => {
    setLoading((prev) => ({ ...prev, auth: true }));
    try {
      const user = await AuthService.registerVerify(email, otp);
      setCurrentUser(user);
      setUserRole(user.role);
    } finally {
      setLoading((prev) => ({ ...prev, auth: false }));
  ...
  const resetPasswordVerify = async (email: string, otp: string, newPassword: string) => {
    setLoading((prev) => ({ ...prev, auth: true }));
    try {
      return await AuthService.resetPasswordVerify(email, otp, newPassword);
    } finally {
      setLoading((prev) => ({ ...prev, auth: false }));
    }
  };
  ...
  const deleteProduct = async (id: string): Promise<boolean> => {
    try {
      const success = await PlantService.deleteProduct(id);
      if (success) {
  ...
  const awardUserCredits = async (userId: string, amount: number): Promise<boolean> => {
    try {
      const res = await UserService.awardCredits(userId, amount);
      if (res.success) {
        setUsers((prev) =>
          prev.map((u) => (u.id === userId ? { ...u, credits: res.newCredits !== undefined ? res.newCredits : (u.credits + amount) } : u))
        );
        return true;
      }
      return false;
    } catch (err) {
      console.error("Lỗi tặng credits:", err);
      return false;
    }
  };
  ...
  const toggleUserStatus = async (userId: string): Promise<boolean> => {
    try {
      ...
      const success = await UserService.updateStatus(userId, nextStatus);
      if (success) {
        setUsers((prev) =>
