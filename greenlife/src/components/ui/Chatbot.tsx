...
import { MessageSquare, X, Send, Bot, User, Sparkles, ChevronDown, Leaf } from "lucide-react";
...
export const Chatbot: React.FC = () => {
  ...
  const handleSendMessage = (text: string, isUserText = true) => {
    ...
    if (isUserText) {
      ...
      // Look for response or give default fallback
      const responseText = BOT_RESPONSES[text] || 
        "Cảm ơn câu hỏi của bạn. Để được hỗ trợ trực tiếp từ đội kỹ thuật sinh vật học, bạn vui lòng liên hệ hotline miễn phí 1800-ECO-GREEN hoặc sử dụng tính năng Đặt Lịch Chuyên Gia nhé.";
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      {isOpen && (
        <div className="w-[340px] sm:w-[380px] h-[480px] bg-stone-950 border border-stone-850 rounded-2xl shadow-2xl flex flex-col overflow-hidden mb-4 animate-in fade-in slide-in-from-bottom-5 duration-300">
          ...
          {/* Header Banner */}
          <div className="bg-emerald-950/80 px-4 py-3 border-b border-emerald-500/10 flex items-center justify-between">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-stone-900/15">
              ...
              >
                ...
                <div className="space-y-1">
                  <div className={`p-3 rounded-2xl text-xs leading-relaxed ${
                    ...
                    {/* Render basic bold text formatting for bot responses */}
                    {msg.text.split("**").map((part, index) => 
                      index % 2 === 1 ? <strong key={index} className={msg.sender === "user" ? "text-black font-extrabold" : "text-emerald-400 font-semibold"}>{part}</strong> : part
                    )}
                  </div>
            ...
            {isTyping && (
              <div className="flex gap-2 max-w-[80%]">
                <div className="w-7 h-7 rounded-full bg-emerald-950 border border-emerald-500/20 text-emerald-400 flex items-center justify-center">
      ...
      <div className="relative group">
        ...
        <button
          ...
          className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl cursor-pointer border relative transition-all duration-300 hover:scale-105 active:scale-95 ${
            isOpen
              ? "bg-stone-950 border-stone-800 text-emerald-400 hover:text-emerald-300"
              : "bg-emerald-500 border-white text-black hover:bg-emerald-400"
          }`}
        ...
        >
          {isOpen ? (
            ...
            <div className="relative w-7 h-7 flex items-center justify-center">
              ...
              <Leaf className="h-3 w-3 text-emerald-950 absolute -top-1 -right-1 fill-emerald-300 stroke-emerald-950 animate-bounce" />
            </div>
          )}
