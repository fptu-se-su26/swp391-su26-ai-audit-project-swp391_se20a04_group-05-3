          ...
          <div className="lg:w-1/2 p-4 sm:p-8">
            ...
            >
              ...
              <button 
                ...
                className="absolute top-6 right-6 p-4 bg-white/90 backdrop-blur-md rounded-full shadow-lg transition-transform hover:scale-110 cursor-pointer"
              >
          ...
          <div className="lg:w-1/2 p-8 lg:p-12 lg:border-l border-slate-100 flex flex-col">
            ...
            <div className="grid grid-cols-2 gap-4 mb-10">
              ...
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                <span className="font-medium text-slate-900">{plant.lightLevel} ({plant.light})</span>
              </div>
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 col-span-2">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1">Nước tưới</span>
                <span className="font-medium text-slate-900">{plant.waterLevel} ({plant.water})</span>
              </div>
            </div>
            ...
            <div className="mt-auto">
              <div className="flex items-center gap-6 mb-6">
                ...
                <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl overflow-hidden h-12">
                  <button 
                    ...
                    className="w-12 h-full flex items-center justify-center text-slate-600 hover:bg-slate-100 hover:text-primary transition-colors cursor-pointer"
                  >-</button>
              ...
              <button 
                ...
                className={`w-full py-5 rounded-2xl font-bold text-lg flex items-center justify-center gap-3 transition-all cursor-pointer ${added ? 'bg-primary text-white' : 'bg-slate-900 text-white hover:bg-primary hover:shadow-xl hover:shadow-primary/20'}`}
              >
                {added ? (
        ...
        <div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {plants.filter(p => p.id !== plant.id).slice(0, 4).map(p => (
              <Link key={p.id} href={`/product/${p.id}`} className="group bg-white p-4 rounded-3xl border border-slate-100 hover:shadow-xl transition-all">
